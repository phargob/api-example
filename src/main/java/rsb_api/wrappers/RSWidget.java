...
public class RSWidget implements Clickable07 {
    ...
    public boolean doAction(final String action, final String option) {
        ...
        for (int i=0; i<3; i++) {
            ...
            ctx.mouse.move(ctx.random(min_x, max_x, rect.width / 5),
                           ctx.random(min_y, max_y, rect.height / 5));

            ctx.sleepRandom(2, 75);
    ...
    public boolean doClick(boolean leftClick) {
        ...
        ctx.mouse.click(ctx.random(min_x, max_x, rect.width / 4),
                        ctx.random(min_y, max_y, rect.height / 4),
                        leftClick);
        return true;
    }
    ...
    public boolean doHover() {
        ...
        ctx.mouse.move(ctx.random(min_x, max_x, rect.width / 4),
                       ctx.random(min_y, max_y, rect.height / 4));
        return true;
    }
     ...
     */
    public RSWidget[] getComponents(boolean nested) {
        if (!isValid()) {
            return null;
        }
