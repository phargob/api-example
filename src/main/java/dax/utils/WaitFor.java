...
public class WaitFor {
    ...
    public static Condition getNotMovingCondition(){
        return new Condition() {
            final WalkerTile initialTile = new WalkerTile(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()));
            final long movingDelay = 1300, startTime = System.currentTimeMillis();

            @Override
            public Return active() {
                if (Timer.timeFromMark(startTime) > movingDelay && initialTile.equals(new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation()))
    ...
    public static int getMovementRandomSleep(Positionable positionable) {
        var playerLoc = new WalkerTile(Ctx.ctx.players.getMyPlayer().getLocation());
        return getMovementRandomSleep((int) Ctx.ctx.calc.distanceBetween(playerLoc,
                                                                         positionable.getLocation()));
    }

    public static int getMovementRandomSleep(int distance) {
        final double multiplier =  Ctx.ctx.walking.isRunEnabled() ? 0.3 : 0.6;
