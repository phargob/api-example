...
public class RSArea {
    ...
    public RSArea(RSTile sw, RSTile ne, int plane) {
        this(new RSTile[]{sw,
                new RSTile(ne.getWorldLocation().getX() + 1, sw.getWorldLocation().getY(), plane),
    ...
    public RSArea(int swX, int swY, int neX, int neY, int p) {
        this(new RSTile(swX, swY, p), new RSTile(neX, neY, p), p);
    }
    ...
    public boolean contains(int x, int y) {
        return this.contains(new RSTile(x, y, plane));
    }
    ...
    public RSTile getCentralTile() {
        ...
        return new RSTile(Math.round(totalX / area.npoints),
                          Math.round(totalY / area.npoints), plane);
    }
        ...
        for (int i = 0; i < this.getWidth(); ++i) {
            for (int j = 0; j < this.getHeight(); ++j) {
                if (this.area.contains(new Point(this.getX() + i, this.getY() + j))) {
                    tiles[i][j] = new RSTile(this.getX() + i,
                                             this.getY() + j,
                                             plane);
                }
            }
