...
import rsb.globval.VarcIntIndices;
import rsb.globval.VarcIntValues;
import rsb.globval.WidgetIndices;
import rsb.globval.enums.InterfaceTab;
import rsb.globval.enums.ViewportLayout;
import rsb.wrappers.*;
...
public class Game {
    ...
    public boolean openTab(InterfaceTab interfaceTab, boolean useHotkey) {
        ...
        if (useHotkey) {
            ...
            var w = ctx.interfaces.getComponent(GlobalWidgetInfo.TO_GROUP(tabWidget.getParent().getId()),
                                                GlobalWidgetInfo.TO_CHILD(tabWidget.getId()));
            w.doClick();
        }
